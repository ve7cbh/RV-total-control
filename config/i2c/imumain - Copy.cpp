...
// =============================================================================
const char* WIFI_SSID     = "VE7CBH_Mikrotik";   // TODO: change to your WiFi SSID
...
void setup() {
  ...
  // OLED address override — this board's silkscreen shows 0x7B (the 8-bit
  // "read" form), which decodes to 7-bit address 0x3D, not the more common
  // 0x3C default. U8g2's setI2CAddress() wants its OWN 8-bit "write" form
  // (address<<1, no R/W bit set) — 0x3D<<1 = 0x7A. Three different-looking
  // numbers (0x7B silkscreen / 0x3D plain 7-bit / 0x7A what U8g2 wants here)
  // all describing the same physical address; this is the one that
  // actually goes in the call below.
  u8g2.setI2CAddress(0x7A);
  if (!u8g2.begin()) {
    Serial.println("WARNING: SH1106 OLED not detected — check wiring; address is now explicitly set to 0x3D (per silkscreen), not the 0x3C default");
  } else {
    Serial.println("SH1106 OLED initialized OK at address 0x3D");
  }
