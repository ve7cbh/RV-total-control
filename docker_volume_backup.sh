...
# ---------------------------------------------------------------------------
BACKUP_DIR="/home/ve7cbh/docker-volume-backups"
LOG_FILE="/var/log/docker_volume_backup.log"
RETAIN_DAYS=7
LOCK_FILE="/run/docker_volume_backup.lock"

# Where to push the finished tarballs on the NAS, replacing BackInTime.
# The script runs as root (via sudo), but the authorized key lives under
# ve7cbh's home dir -- point rsync/ssh at it explicitly rather than relying
# on root's own (probably nonexistent) ~/.ssh.
REMOTE_USER="ve7cbh"
REMOTE_HOST="192.168.88.4"
REMOTE_PATH="/volume1/rvtc/docker-volume-backups"
SSH_KEY="/root/.ssh/id_rsa"
# Set to 0 to skip the NAS push entirely (e.g. testing locally only)
PUSH_TO_NAS=1
...
# --- 6. Push to NAS via rsync-over-ssh (replaces BackInTime) -----------------
if [ "$PUSH_TO_NAS" -eq 1 ]; then
    log "Pushing backups to ${REMOTE_USER}@${REMOTE_HOST}:${REMOTE_PATH}"
    if rsync -avz --delete \
        -e "ssh -i $SSH_KEY" \
        "$BACKUP_DIR"/ \
        "${REMOTE_USER}@${REMOTE_HOST}:${REMOTE_PATH}/" >>"$LOG_FILE" 2>&1; then
        log "NAS push succeeded"
    else
        log "NAS push FAILED — see log above for rsync error"
        FAILED="$FAILED [nas-push]"
    fi
fi
