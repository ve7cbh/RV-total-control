...
**Version 1.5** — supersedes Version 1.4. Changes in this revision:
**2026-07-16 WN90LP (HW-16) bench bring-up session added** — sensor
confirmed alive and reading correctly over both direct-USB and the real
Waveshare gateway path, using a temporary bench connection on `485-6`
(borrowed from HW-24's freed wiring; not a permanent reassignment). Root
causes for the day's earlier "won't talk at all" and later "intermittent
CRC failures" symptoms both resolved: a gateway port-baud mismatch left
over from mid-session troubleshooting, and — the more interesting one —
two simultaneous Modbus masters (the vendor Windows tool's own
undocumented free-running auto-poll, plus CLI/script test traffic)
fighting over one RS-485 pair. See
[2026-07-16 log](#2026-07-16--wn90lp-bench-bring-up-and-modbus-troubleshooting)
for full detail, and updated entries in [3.1](#31-hardware) (HW-16, HW-24),
[5.1](#51-device--gateway-inventory), and [5.5](#55-rs-485-bus--gateway-ip-addressing).
Next step: temporary roof placement to check rain measurement against
current conditions before finalizing mount location and running permanent
`485-7` wiring.

---

**Version 1.3** — supersedes Version 1.2. Changes in this revision: **HW-25
IMU node implementation superseded.** The ESP32 + Adafruit 10-DOF (LSM303DLHC/
L3GD20/BMP180) approach — fully bring-up-confirmed as of 2026-07-10 — was
abandoned on 2026-07-13 after a full-day mounting/calibration session made
clear the enclosure's own contents (ESP32 RF shield can, RS-485 level
converter, DC-DC converter, all sharing one small box with the sensor) were
the dominant source of an uncalibratable hard-iron bias, no matter how good
the calibration technique got. Replaced by a plan to mount a **WitMotion
WTGAHRS3-485 GPS-IMU** in its own stainless/aluminum enclosure on the roof
(wood-truss structure — magnetically clean, unlike the trailer chassis) as a
native Modbus RTU slave, sharing the existing RS-485 run already in place for
the Ecowitt WS90 weather station. This eliminates the ESP32/custom-firmware
layer for this sensor entirely — no more axis-swap firmware, no more OTA/
OLED/web code, no microcontroller in the loop at all — at the cost of losing
the standalone OLED display and local web page (bench conveniences only).
**This is a design decision, not yet a completed migration** — see
[Part 4 — 2026-07-13](#2026-07-13--imu-implementation-change-esp32adafruit-10-dof-abandoned)
for the full story and [Part 3](#31-hardware) for what's still open.
...
node build; 2026-07-10 with the IMU node's full hardware bring-up,
confirmed working end-to-end on both its RS-485 and WiFi/MQTT paths; and
2026-07-13 with the decision to abandon that implementation in favor of a
roof-mounted WTGAHRS3-485 — see
...
- [Part 4 — Chronological Session Log](#part-4--chronological-session-log)
  ...
  - [2026-07-13 — IMU Implementation Change: ESP32/Adafruit 10-DOF Abandoned](#2026-07-13--imu-implementation-change-esp32adafruit-10-dof-abandoned)
  - [2026-07-16 — WN90LP Bench Bring-Up and Modbus Troubleshooting](#2026-07-16--wn90lp-bench-bring-up-and-modbus-troubleshooting)
...
This topic-tree/no-persistence decision still holds regardless of which
physical sensor publishes to it — **see 2026-07-13** below for the hardware
change; the `rvtc/sensors/imu/#` schema itself is not affected, though the
GPS lat/long/speed fields the WTGAHRS3-485 adds will need a schema decision
of their own (real-time-only like heading/pitch/roll, or persisted, since
position history has different value than attitude history) — not yet made.
...
| HW-10 | Install GNSS E108-GN03G-485 (RS-485/6, device in hand) | ✅ Closed 2026-07-07 — module replaced, `gps_mqtt.py` deployed and confirmed publishing. ⛔ **Decommissioned 2026-07-14** — superseded by the WTGAHRS3-485 combined GPS-IMU unit (HW-27); `485-6` repurposed for the 400A current sensor (HW-24). `gps_mqtt.service` and `gpsd.service`/`gpsd.socket` stopped and disabled same day. |
...
| HW-16 | Ecowitt WN90LP weather station (FG-WN90LP-EC01, in hand — RS-485/7 nominal) | Open — **naming resolved 2026-07-13**: WN90LP is confirmed as the wired RS-485 variant of the WS90 7-in-1 sensor array (solar panel replaced with an RS-485 interface, plus an added heating element) — same sensor, same `WS90ModbusRTU` protocol document, same default slave `0x90` reviewed in `WS90ModbusRTU_V1_0_6.pdf`. HW-27's shared-bus plan is confirmed describing the same physical hardware as this entry. **2026-07-16 bench bring-up — ✅ confirmed working end-to-end**, but on `485-6` (`192.168.88.10:4001`) rather than its nominal `485-7`, as a deliberate **temporary** bench test — used HW-24's freed-up port-6 wiring since it was easiest to reach, not a permanent reassignment. Still needs its own proper run to `485-7` before roof install. See [2026-07-16 log](#2026-07-16--wn90lp-bench-bring-up-and-modbus-troubleshooting) for full troubleshooting detail (gateway port baud, dual-master CRC contention, working pymodbus script). |
...
| HW-24 | DC shunt 400A RS-485 (ordered) | ⏳ In progress 2026-07-14 — port `485-6` freed (GPS decommissioned, HW-10), sensor being wired in now; see [2.1](#21-epever-battery-soc-swings--resolved), true fix for voltage-based SOC limitation. **Note 2026-07-16:** port `485-6`'s wiring was temporarily borrowed for an unrelated WN90LP (HW-16) bench bring-up session — see that entry and the 2026-07-16 log. Not a permanent reassignment; HW-24 still owns `485-6` long-term. |
| HW-25 | IMU node — NodeMCU ESP32 + Adafruit 10-DOF (LSM303DLHC/L3GD20/BMP180) + SH1106 OLED | ⛔ **Superseded 2026-07-13** — implementation abandoned, not merely closed. Bring-up itself was genuinely completed 2026-07-10 (both paths confirmed working — see that log), but a full day of mounting/hard-iron calibration work made clear the enclosure's own contents (ESP32 RF can, RS-485 converter, DC-DC converter, all a few cm from the sensor) were an uncalibratable magnetic interference source regardless of technique. Board and firmware set aside; see HW-27/HW-28 below and [2026-07-13 log](#2026-07-13--imu-implementation-change-esp32adafruit-10-dof-abandoned) for the full story. |
| HW-27 | **NEW** — WitMotion WTGAHRS3-485 GPS-IMU, roof-mounted (wood truss — magnetically clean), replaces HW-25. Native Modbus RTU slave — no microcontroller/custom firmware in front of it, unlike HW-25's ESP32 approach | Open — design decided 2026-07-13, not yet purchased/mounted. Open sub-items: (1) confirm exact register map against the real **WTGAHRS3-485 Communication Protocol** document (general WitMotion protocol table found so far has an unresolved address collision between `HZ`/`Roll` at register 61 — see [2026-07-13 log](#2026-07-13--imu-implementation-change-esp32adafruit-10-dof-abandoned)); (2) locate GPS lat/long/ground-speed register addresses (not in any doc found yet); (3) reassign the module's Modbus slave ID away from its factory default `0x50` before wiring onto the shared bus (must not collide with the Ecowitt WS90's `0x90`/144, or anything else on that run); (4) confirm the gateway IP:port and baud/parity this shared RS-485 run is actually configured for; (5) verify the 12V→5V DC-DC supply's actual output is within this module's stricter ≤5V limit (its manual states over-5V "can lead to permanent damage," a tighter tolerance than typical hobby breakouts) before connecting it; (6) run the module's own built-in hard-iron calibration command sequence (send start-cal, rotate through 3 axes, send end-cal) once mounted in its final position — this replaces the MotionCal/serial-streaming workflow HW-25 required, per WitMotion's SDK docs. See `MODBUS_IMU.py` (drafted 2026-07-13, register addresses marked as unconfirmed placeholders) |
| HW-28 | **NEW** — Decommission HW-25 ESP32 IMU node once HW-27 is operational | Open — decision needed on whether to keep the ESP32 node's OLED/local-web-page as a bench-only convenience elsewhere, or fully retire the hardware. Not urgent; blocked on HW-27 |
...
### 2026-07-13 — IMU Implementation Change: ESP32/Adafruit 10-DOF Abandoned

**Started as a mounting/axis-orientation session, ended as an architecture
decision.** Session picked up from the 2026-07-10 bring-up with the IMU
mounted "perhaps not final" — HW-25 firmware and axis handling itself
proved correct all session, but ultimately not the reason the implementation
was abandoned.

**Axis/orientation firmware fixes (all landed successfully before the
architecture decision, kept here for the record even though HW-25 is
superseded):**
- Confirmed via physical sign-check that the original mount (X-axis
  parallel to centerline) had roll/pitch mapped correctly — no firmware
  change needed at that point.
- Physically rotated the enclosure 90° CW (viewed down Z) so X aligned with
  centerline per the project's "normal" convention. This swap required
  negating one axis (`raw_pitch = orientation.roll; raw_roll =
  -orientation.pitch;`) — confirmed by hand-test (bow-up → old "Roll" field
  increased; stbd-down → old "Pitch" field decreased), not assumed from
  theory alone.
- Added static trim constants (`PITCH_OFFSET_DEG -2.12`, `ROLL_OFFSET_DEG
  -3.15`, `HEADING_OFFSET_DEG -176.7`) derived from one known-position
  reading, explicitly documented as rough/provisional pending final mount
  and real hard-iron cal.
- Added OTA (`ArduinoOTA`, password shared with WiFi credentials — a
  deliberate convenience/security tradeoff, not an oversight).
- Added a temporary MotionCal-compatible raw serial (`Raw:` line) output
  block for hard-iron calibration, clearly delimited for later removal.

**Hard-iron calibration — the long middle of the session, ultimately
diagnostic rather than successful.** Roughly 20+ MotionCal attempts across
several hours, producing lines/arcs/ribbons/domes instead of a filled
sphere. Root causes identified over the course of troubleshooting, in
order of appearance:
1. A copper wire routed across the IMU was inducing real magnetic
   interference — rerouting it produced a large, confirmed improvement
   (heading tracking became continuous and directionally correct through a
   full 360° hand-rotation test, cardinal points landing in roughly the
   right places).
2. Technique problems compounded the remaining difficulty for hours: free-
   hand rotation was repeatedly, unknowingly collapsing to near-single-axis
   motion (flat spins constrained by resting on a table; large-radius arm
   swings that translate through space without meaningfully changing the
   sensor's *attitude*; "flips" that were mostly one type — e.g.
   end-over-end — repeated rather than genuinely cycling through
   end-over-end/side-over-side/spin-in-place). A session run outdoors,
   after deliberately cycling all three flip types as distinct, separate
   actions, finally produced a real filling sphere (Fit Error 12%, clean
   identity mapping) — proving the technique problem, not the hardware, was
   the dominant remaining obstacle by that point.
3. Once technique was solid, attention returned to what's actually inside
   the enclosure: **ESP32 module (with its RF shielding can), RS-485 level
   converter, and 12V→5V DC-DC converter, all sharing one small box a few
   centimeters from the magnetometer.** This is a well-known class of hard-
   iron interference source, and unlike the copper wire, isn't something
   that can be rerouted away without a fundamentally different enclosure
   layout — these components need to physically travel with the sensor.

**The decision:** rather than continue fighting a mounting/enclosure
problem with an inexpensive, already-debugged sensor, abandon the
ESP32+Adafruit approach and replace it with a **WitMotion WTGAHRS3-485
GPS-IMU** mounted in its own enclosure on the RV's roof, which has **wood
truss framing** — magnetically inert, unlike the aluminum-and-steel
enclosure/chassis environment HW-25 was fighting all day. This also
resolves two other open concerns for free: GPS antenna sky-view (roof is
about the best possible placement) and the IP67 rating on the module
finally being load-bearing (roof-exposed, unlike HW-25's interior plastic
box). Explicitly framed as *"discarding a $12 IMU and several days' labor
worth more than that — an IMU in a stainless container on the roof is
worth the money,"* i.e. a considered trade, not a sunk-cost mistake.

**Architecture implications, worked through in this session:**
- WTGAHRS3-485 is a **native Modbus RTU slave** — no microcontroller sits
  in front of it. This eliminates the entire HW-25 firmware layer (axis
  math, OTA, OLED, local web page) for this sensor. It integrates the same
  way KWS-303L/EPEVER/SAMLUX do: a Python bridge script polling through the
  Waveshare gateway, not a custom ESP32 node.
- Confirmed (from the WTGAHRS3-485's own manual) that its onboard
  algorithm outputs **already-computed Roll/Pitch/Yaw** as Modbus holding
  registers — no `Adafruit_10DOF`-style accel/mag fusion math needed at
  all in the new design.
- Confirmed the module has a **built-in hard-iron calibration command**
  (start-cal → rotate through 3 axes → end-cal), replacing the whole
  MotionCal/serial-streaming workflow this session spent hours on. The
  physical motion required is the same; the software side collapses to two
  commands instead of a PC app and manual constant transcription.
- Plan to **share the existing RS-485 run already wired to the roof** for
  the Ecowitt weather station, rather than adding a second cable run.
  Confirmed via `WS90ModbusRTU_V1_0_6.pdf` that the weather sensor
  (`WS90`, default slave `0x90`, 9600 8N1, standard Modbus RTU function
  codes 0x03/0x06) is genuine standard Modbus — not a proprietary protocol
  — so multi-dropping the IMU module onto the same physical pair is
  architecturally sound, pending the open items below.
- Drafted `MODBUS_IMU.py` (see [3.1](#31-hardware) HW-27 for the full list
  of unconfirmed placeholders it still contains) following the same
  structure as `kws_mqtt.py`, adding `to_signed16()` handling that the
  all-positive KWS register set never needed — IMU angle/accel/gyro values
  routinely go negative.

**Explicitly NOT yet done:** no hardware purchased, no register map
confirmed against the real WTGAHRS3-485-specific protocol document (only
WitMotion's general cross-product-line register table was found — it has
an internal contradiction, register 61 appearing as both `HZ` and `Roll` in
different sources, that needs resolving against the real document), no GPS
register addresses located, no gateway IP/slave-ID assignment done. HW-25
hardware and firmware are set aside, not discarded from the repo — the
firmware fixes from earlier in this same session remain a correct, working
reference for the axis-orientation problem class even though the sensor
itself is being replaced.

### 2026-07-16 — WN90LP Bench Bring-Up and Modbus Troubleshooting

**Goal:** get the Ecowitt WN90LP (HW-16) talking over Modbus RTU/TCP as a
first step before roof mounting, using whatever bus was easiest to reach on
the bench — ended up temporarily using the wiring freed up for the 400A
current sensor (HW-24, `485-6`, `192.168.88.10:4001`) rather than the
WN90LP's nominal `485-7`, purely for bench convenience. **Not a permanent
reassignment** — HW-24 still owns `485-6` long-term; the WN90LP needs its
own proper run to `485-7` before actual roof install.

**Root causes chased down, in the order they were found:**

1. **Initial total silence (`mbpoll` timeouts) — several causes stacked:**
   - `mbpoll -t 4` vs `-t 3` confusion: mbpoll's `-t` flag is its own type
     enum, not the raw Modbus function code — `-t 4` = 16-bit **holding**
     register (Modbus function 0x03), `-t 3` = **input** register (function
     0x04). The WN90LP/WS90 only implements function codes 0x03/0x06 per
     `WS90ModbusRTU_V1_0_6.pdf`, so `-t 4` is the correct flag. (Went down a
     wrong path mid-session suggesting `-t 3` — corrected once tested
     against a known-working device confirmed mbpoll/network path were
     fine and isolated the issue elsewhere.)
   - `mbpoll -r` is 1-based and subtracts 1 before putting the address on
     the wire — reading documented register `0165H` needs `-r 358`, not
     `-r 357`/`-r 1`.
   - Wrong IP initially assumed (`485-7`/`.11`) before confirming the bench
     session was actually wired to `485-6`/`.10`.
2. **Sensor confirmed alive via the vendor "Ecowitt Modbus RTU" Windows
   tool** (direct USB-RS485, bypassing the gateway) — first real bytes back
   were a status-check frame (`FD FD FD ...`, per the doc's §1.4.2 "Special"
   command) showing an invalid/unset state, then a `Read data` click
   produced a real `90 03 1c ...` reply, confirming slave `0x90`, 9600 baud,
   and valid in-range sensor data (temp, humidity, pressure, wind all
   plausible).
3. **Persistent intermittent CRC-failure replies** (`90 83 08 11 1b` —
   function 0x03 exception, error code 08 = CRC fail per the doc's §1.5
   error table) even once basic comms worked, roughly 20-30% of polls.
   Multiple theories tested and ruled out in order: over-polling rate,
   physical splice/connection quality, EMI/termination. **Actual cause:
   two simultaneous Modbus masters on one RS-485 pair** — the vendor
   Windows tool has its own internal auto-poll loop (confirmed by clearing
   its log and disconnecting nothing — it kept producing fresh frames on
   its own, roughly matching the sensor's ~8.75s internal refresh rate per
   §1.3's remarks) with no visible UI toggle to disable it. Once the
   Windows tool was fully closed and only a single Linux-side script was
   master on the bus, CRC failures disappeared completely and reads were
   100% clean and repeatable.
4. **One more real gateway-side issue found after moving to the actual
   Waveshare gateway path** (rather than direct-USB): the gateway's
   configured baud for that port had been left at 4800 from earlier
   same-session troubleshooting (not a GPS-era leftover as first assumed —
   just a setting changed and not reverted mid-session). Setting the
   gateway port back to 9600 (matching the WN90LP's default) resolved it;
   reads through the real gateway path are now clean.

**Working confirmed stack:** Python 3, `pymodbus` 3.13.1,
`ModbusTcpClient`, function 0x03 (`read_holding_registers`), registers
`0165H`-`016DH` (9 registers: light, UVI, temp, humidity, wind, gust, wind
dir, rain, ABS pressure), against `192.168.88.10:4001` (bench-temporary,
via `485-6`), slave/device ID `144` (`0x90`). **Note:** pymodbus 3.13.x
uses the `device_id=` keyword, not the older `slave=` kwarg seen in most
online examples/docs for earlier 3.x releases — this bit us mid-session
and is worth remembering for anything else built against this pymodbus
version. Read-with-retry logic (3 attempts, 0.5s backoff) added as a
matter of course, independent of root cause, since occasional single-poll
failures are normal on any RS-485 bus.

**Explicitly NOT yet done:** no production `WN90_mqtt.py` bridge script
written yet (only an ad-hoc bench test script, `testmodbuss`); no
permanent `485-7` wiring run to the WN90LP's actual mounting position; no
roof mount; no confirmation yet of behavior under real outdoor conditions
(rain, wind, temperature extremes) — next planned step is a temporary
roof placement to confirm rain measurement against current weather before
committing to final mount location and permanent wiring.
...
| ⛔ **DECOMMISSIONED 2026-07-14** — GNSS E108-GN03G-485 | RS-485 bus 6 (`485-6`, confirmed via HW-10 cross-reference) | `192.168.88.10:4001` — TCP serial gateway, module replaced 2026-07-06/07 with a working u-blox unit | `gps_mqtt.py` — **not a direct gateway poller**; ran as a client of the host's own `gpsd` (port 2947), which was gpsd's single TCP client of the gateway. Deliberate departure from the direct-poll pattern used by the other bridges — see script header comment for reasoning. **Superseded by the WTGAHRS3-485 combined GPS-IMU (HW-27, native Modbus RTU slave) — the gpsd-as-intermediary architecture was specific to the standalone GNSS module's own protocol and does not carry over to a Modbus-native device, so it is not being reused.** | `gps_mqtt.service` — stopped, disabled 2026-07-14. `gpsd.service`/`gpsd.socket` — stopped, disabled 2026-07-14 (confirmed this, not `gps_mqtt.service`, was holding the TCP session to the gateway; port-open LED cleared only after `gpsd` was stopped). | `rvtc/sensors/gps/#` (`latitude`, `longitude`, `altitude_m`, `speed_kmh`, `track_deg`, `time_utc`, `fix_mode`, `hdop`, `satellites_used`, `satellites_visible`, `availability`) — retired; GPS fields to be re-established under the WTGAHRS3 topic, see row below |
| **NEW — 400 amp battery current sensor** (HW-24) | RS-485 bus 6 (`485-6`) — reassigned from the decommissioned GNSS module above. **Temporarily lent to the WN90LP bench test 2026-07-16 (see row below and 2026-07-16 log) — reclaim for HW-24 install once the WN90LP moves to its own `485-7` run.** | `192.168.88.10:4001` — same Waveshare gateway, port freed 2026-07-14 | *(not yet written — install in progress)* | *(not yet deployed)* | *(planned — likely `rvtc/sensors/battery_current/#` or folded into existing battery/power topic tree, TBD)* |
| Ecowitt WN90LP weather station | Nominal home: RS-485 bus 7 (`485-7`, confirmed via HW-16 cross-reference). **As of 2026-07-16, temporarily bench-tested on `485-6` instead** (borrowed HW-24's port-6 wiring — easiest to reach on the bench; not permanent) | Nominal `192.168.88.11` (`485-7`, not yet wired); **bench-tested successfully at `192.168.88.10:4001`** (`485-6`) 2026-07-16 | `testmodbuss` (ad-hoc bench script, pymodbus 3.13.1, `ModbusTcpClient`, function 0x03, registers `0165H`-`016DH`, retry-on-error loop) — **not yet the production bridge**; `WN90_mqtt.py` still to be written | — | *(likely `rvtc/sensors/weather/#`, pending `weewx-mqtt`)* |
| MT50 remote | RS485-1M2S slave port, shares EPEVER line | n/a (direct RS-485, not TCP) | n/a | n/a | n/a |
| ⛔ **SUPERSEDED 2026-07-13** — IMU node — NodeMCU ESP32 (plain, not S3 — 30-pin variant, no GPIO16/17 broken out) + Adafruit 10-DOF + SH1106 OLED | RS-485 bus 4 (`485-4`) — was **PRIMARY path**, dedicated gateway port, ✅ confirmed working end-to-end 2026-07-10 via real Modbus read | `192.168.88.8:4001` | `main.cpp` (PlatformIO, C++, not Python like the other bridges) — repo: `rvtc-imu/`. RS-485 pins: RX=GPIO18, TX=GPIO19, DE/RE=GPIO4 (moved off 16/17 — not present on this board). WiFi + MQTT + local web page are secondary/convenience paths only, matching [1.5](#15-rs-485--modbus-gateway-topology)'s general principle. OLED confirmed at I2C address 0x3C (standard default — no override needed; an earlier assumed 0x3D was wrong). **Hardware/firmware are fully bring-up-confirmed and functionally correct — abandoned for a mounting/hard-iron-interference reason (enclosure contents, not a bug), see [2026-07-13 log](#2026-07-13--imu-implementation-change-esp32adafruit-10-dof-abandoned) and HW-27/HW-28 in [3.1](#31-hardware).** | *(runs as the ESP32's own firmware, no host-side systemd service)* | Modbus holding registers 0-2 (heading/pitch/roll, ×10 scaled int16) via RS-485; `rvtc/sensors/imu/#` (`heading`, `pitch`, `roll`, `heave`, `status`) via MQTT — **not persisted to InfluxDB**, real-time only |
| **NEW, replaces above** — WitMotion WTGAHRS3-485 GPS-IMU, planned roof mount (wood truss) | RS-485 — planned to share the existing run to the Ecowitt WS90 weather station | TBD — gateway IP:port not yet confirmed | Native Modbus RTU slave, no microcontroller/custom firmware. **Decided 2026-07-14: will follow the standard direct-poll bridge pattern (Modbus master polling the gateway directly), same as EPEVER/SAMLUX/KWS — the `gpsd` intermediary layer used for the old standalone GNSS module (E108-GN03G-485, decommissioned same day) is a non-starter here since gpsd expects a raw NMEA/serial GPS device, not a Modbus register map.** See HW-27 in [3.1](#31-hardware) for the full open-items list and `MODBUS_IMU.py` (drafted 2026-07-13, register addresses unconfirmed) | `MODBUS_IMU.py` (drafted, not yet deployable — see HW-27) | Planned: `rvtc/sensors/imu/#` (same topic tree as the superseded node) plus new GPS lat/long/speed fields — schema for the GPS fields not yet decided (see [1.2](#12-unified-namespace-mqtt)) |
...
| `485-4` | `192.168.88.8` | IMU node** (HW-25)|  ⛔  Connected, FAILED — Unable to CAL.  To be replaced with WTGAHRS3 IMU + GPS See RVTC_SessionNotes_20260713_IMU_Implementation_Change.md|
| `485-5` | `192.168.88.9` | **Confirmed: Water sensors** — pressure + filter ΔP + turbidity | Pending Phase 5 |
| `485-6` | `192.168.88.10` | **Home: 400 amp battery current sensor** (HW-24). Was GNSS E108-GN03G-485 (HW-10) — GPS decommissioned 2026-07-14 to free the port; see device inventory row below and HW-10 in [3.1](#31-hardware). `gpsd.service`/`gpsd.socket` stopped and disabled same day, confirmed gateway port-open LED cleared. **2026-07-16: temporarily carrying the WN90LP (HW-16) instead, for bench bring-up/testing only — see 2026-07-16 log.** | ⏳ HW-24 install still pending — port currently occupied by temporary WN90LP bench test as of 2026-07-16, reclaim once WN90LP moves to its own `485-7` run |
| `485-7` | `192.168.88.11` | **Confirmed: Ecowitt WN90LP weather station** (nominal/permanent home) | Shipped; **not yet wired to this port** — currently bench-tested on `485-6` instead (temporary, see HW-16). Real `485-7` run + roof mount still open. |
...
**IMU node (HW-25) — SUPERSEDED 2026-07-13** → [5.1](#51-device--gateway-inventory), [1.5](#15-rs-485--modbus-gateway-topology), [3.1](#31-hardware), [2026-07-09 log](#2026-07-09--imu-node-build-hw-25), [2026-07-10 bring-up log](#2026-07-10--imu-node-bring-up-hw-25--confirmed-working-end-to-end), [2026-07-13 log](#2026-07-13--imu-implementation-change-esp32adafruit-10-dof-abandoned)

**WTGAHRS3-485 / roof IMU-GPS (HW-27, replaces HW-25)** → [3.1](#31-hardware), [5.1](#51-device--gateway-inventory), [2026-07-13 log](#2026-07-13--imu-implementation-change-esp32adafruit-10-dof-abandoned)

**MODBUS_IMU.py (drafted, unconfirmed placeholders)** → [3.1](#31-hardware), [2026-07-13 log](#2026-07-13--imu-implementation-change-esp32adafruit-10-dof-abandoned)

**Hard-iron calibration technique (flip-type cycling, MotionCal)** → [2026-07-13 log](#2026-07-13--imu-implementation-change-esp32adafruit-10-dof-abandoned)

**WS90 / Ecowitt Modbus RTU (shared RS-485 run)** → [3.1](#31-hardware), [2026-07-13 log](#2026-07-13--imu-implementation-change-esp32adafruit-10-dof-abandoned), [2026-07-16 bench bring-up log](#2026-07-16--wn90lp-bench-bring-up-and-modbus-troubleshooting), [5.1](#51-device--gateway-inventory), [5.5](#55-rs-485-bus--gateway-ip-addressing)

**WN90LP bench bring-up / Modbus troubleshooting (2026-07-16)** → [2026-07-16 log](#2026-07-16--wn90lp-bench-bring-up-and-modbus-troubleshooting), [3.1](#31-hardware) (HW-16), [5.1](#51-device--gateway-inventory), [5.5](#55-rs-485-bus--gateway-ip-addressing)

**pymodbus `device_id=` vs `slave=` kwarg (3.13.x API change)** → [2026-07-16 log](#2026-07-16--wn90lp-bench-bring-up-and-modbus-troubleshooting)

**mbpoll `-t`/`-r` flag gotchas (type enum, 1-based register offset)** → [2026-07-16 log](#2026-07-16--wn90lp-bench-bring-up-and-modbus-troubleshooting)

**Dual-master RS-485 contention (vendor tool auto-poll)** → [2026-07-16 log](#2026-07-16--wn90lp-bench-bring-up-and-modbus-troubleshooting)
