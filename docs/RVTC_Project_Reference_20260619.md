...
- HSR1-25 25A NO relay × 2 — water heater and fridge AC connection (HW-18) — **Normally Open**: coil
- energised = AC connected; coil de-energised = LPG fallback
...
- **Map orientation correction** — position display pages and dashboards can orient correctly
- without manual compass input.
...
(figure-eight rotation, constants baked into ESPHome YAML) removes the fixed offset. The calibration
...
**Notes:**
- The gyroscope and accelerometer axes are available for transit shock/vibration logging and RV
- levelling feedback — secondary use cases, not the primary driver
- The barometric sensor sometimes bundled with this module is superseded by WN90LP (HW-16) — ignore
- if present
- ESPHome does not include a built-in LSM303D calibration wizard; run a short Python script against
- raw I²C readings to derive hard-iron offsets, then set as static calibration constants in YAML
...
- Addresses: 1–8, 1-based, no offset (no `-0` flag needed for coil addressing)
- Verified: `mbpoll -m tcp -a 1 -t 0 -r 1 -c 8 192.168.88.12 -p 4001`
...
**Complete coil assignment:**

| Coil | Load | Contact type | Voltage switched | Normal state on grid/gen | Control tier |
|---|---|---|---|---|---|
| 1 | Water heater AC | HSR1-25 NO relay | 120VAC | Energised | Tier 1, Tier 3 |
| 2 | Fridge AC | HSR1-25 NO relay | 120VAC | Energised | Tier 1, Tier 3 |
| 3 | Spare | — | — | — | Future use |
| 4 | Spare | — | — | — | Future use |
| 5 | EVO-2212 BMS charge inhibit | Direct NO contact | Potential-free (EVO port 6, pins 1+5) | Open (charging allowed) | Tier 2 |
| 6 | Battery heater (120VAC seed mat) | Direct NO contact | 120VAC (<1A) | De-energised | Tier 2 |
| 7 | Spare | — | — | — | Future |
| 8 | Spare | — | — | — | Future |
...
- Coil 1 off → water heater to LPG (instant, no consequence)
- Coil 2 off → fridge to LPG (instant, no consequence)
- ESP32 sends CCC off command → A/C controlled shutdown (A/C board manages its own
  compressor sequence including K1 delay)

Restore: EVO returns to passthrough/charging mode
- Coil 1 on → water heater to AC
- Coil 2 on → fridge to AC
- ESP32 sends CCC on command (after 5-minute guard) → A/C restart — K1 hardware delay
  in A/C unit provides additional protection

Rationale: When on inverter, water heater and fridge have LPG fallback at zero cost. A/C running on
batteries is a major draw — controlled shutdown is appropriate. EVO's 1–3 ms transfer means
appliances never see a power interruption.
...
- Step 1: Coil 1 off (water heater). Wait 60s. Re-measure.
- Step 2: If still >22A → coil 2 off (fridge). Wait 60s. Re-measure.
- Step 3: If still >20A AND A/C running → ESP32 CCC off command (A/C controlled
  shutdown). Last resort only.
- Log event.
- Restore water heater: current <20A → coil 1 on
- Restore fridge: current <20A with WH restored → coil 2 on
- Restore A/C: current <18A sustained 3 min AND 5-minute guard elapsed → ESP32
  CCC on command

Generator overload (KWS-303L slave 2, reg 18):
- Trigger: >22A sustained 30s
- Step 1: Coil 1 off (water heater). Wait 60s.
- Step 2: If still >19A → coil 2 off (fridge). Wait 60s.
- Step 3: If still >17A AND A/C running → ESP32 CCC off command. Last resort.
  Log event.
- Restore thresholds: WH <18A, fridge <18A, A/C <16A sustained 3 min + 5-min guard
...
> **NOTE:** New RSC (ether8 + pinhole fixes) committed but not yet loaded onto physical router —
> install and test at next visit.
...
| HW-13 | 3 | Waveshare 8-ch RS-485 Modbus relay board — shore power load management | ✅ Closed | Commissioned 2026-06-19 — all 8 coils verified, cycle test passed. Coil 1 = water heater, coil 2 = fridge. NO configuration. Port 4001, slave 1, IP 192.168.88.12. HSR1-25 physical wiring pending (HW-18). |
...
| HW-18 | 3 | Install HSR1-25 25A NO relay × 2 — water heater AC and fridge AC | 🟡 Open | **Ordered 2026-06-13** — wired Normally Open (changed from NC); coil energised = AC connected. Channel 1 = water heater, channel 2 = fridge. Driven by Waveshare 8-ch relay board (HW-13 ✅). Physical wiring in progress 2026-06-19. |
...
1. HSR1-25 physical wiring — complete installation of NO relays for water heater and fridge AC
(HW-18)
2. Verify CCC connector pin functions with meter — confirm furnace demand wire and A/C demand wire
before connecting coils 3 and 4
4. Wire coil 5 (EVO BMS charge inhibit) — RJ-45 plug to EVO port 6, pins 1+5 (HW-21)
5. Install battery heater and wire to coil 6 (HW-22)
6. Fish DMX cable to thermostat location — RS-485/4 (HW-20)
7. Generator meter — set slave 2 on bench, wire onto RS-485/3 shared bus (HW-09)
8. EPEVER MPPT60 Modbus integration (RS-485/1, IP 192.168.88.5, TCP 4001)
9. HA modbus_relay.yaml — coil control for Tier 3 load-shed automation
10. Home Assistant onboarding — MQTT integration (OI-15)
11. Rebuild Grafana dashboard (OI-16)
12. WN90LP commissioning when received (HW-16)
