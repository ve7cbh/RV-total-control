...
| RTL-SDR primary | RTL-SDR Blog V3 — R828D tuner, TCXO, SMA, aluminium case — SN 00000001 — **active** |
| RTL-SDR spare | RTL2838 DVB-T clone — R820T tuner — failing (USB instability) — set aside |
...
|---|---|---|---|
| RTL-SDR Blog V3 | R828D, TCXO, SMA, aluminium | 00000001 | Active — in J45 |
| Clone (old primary) | R820T | 00000001 | Failing — USB instability — set aside |

**Notes:**
- PLL not locked warning on R828D is benign — always present, does not affect reception
...
**Container image:** `hertzg/rtl_433:latest`
**Container command:**
```
-M si -F "mqtt://mosquitto:1883,retain=1,events=rtl_433/rtl433/events"
```
**MQTT topic:** `rtl_433/rtl433/events` (JSON)
...
| OI-26 | rtl433 container device addressing | 2 | ✅ Complete | Resolved by passing /dev/bus/usb whole bus |
...
| HW-11 | RTL-SDR Blog V3 | 3 | ✅ Complete | Received and working — R828D tuner confirmed |
...
- **MQTT topic:** rtl_433 publishes to `rtl_433/rtl433/events`. WeeWX subscribes to `rtl_433/+/events` (wildcard match). `+` is valid in subscribe but NOT in publish topics.
...
- RTL-SDR Blog V3 (HW-11) confirmed working — R828D tuner
- Old clone dongle confirmed failing — set aside
